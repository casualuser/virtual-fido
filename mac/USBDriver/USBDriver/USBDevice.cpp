...
constexpr unsigned char fidoReportDescriptor[] = {
    6,  208, 241, 9, 1,  161, 1,  9,   32,  20, 37,  255, 117, 8, 149,
    64, 129, 2,   9, 33, 20,  37, 255, 117, 8,  149, 64,  145, 2, 192};
...
OSDictionary *USBDevice::newDeviceDescription(void) {
  ...
  struct KV {
    ...
    OSObject *value;
      ...
      {kIOHIDTransportKey, OSString::withCString("Virtual")},
      {kIOHIDManufacturerKey, OSString::withCString("VirtualFIDO")},
      {kIOHIDVersionNumberKey, OSNumber::withNumber(1, 8 * sizeof(1))},
      {kIOHIDProductKey, OSString::withCString("VirtualFIDO")},
      {kIOHIDSerialNumberKey, OSString::withCString("123")},

      {kIOHIDVendorIDKey, OSNumber::withNumber(123, 32)},
      {kIOHIDProductIDKey, OSNumber::withNumber(123, 32)},
      {kIOHIDLocationIDKey, OSNumber::withNumber(123, 32)},
      {kIOHIDCountryCodeKey, OSNumber::withNumber(840, 32)},
      {kIOHIDPrimaryUsagePageKey, OSNumber::withNumber(kHIDPage_FIDO, 32)},
      {kIOHIDPrimaryUsageKey,
       OSNumber::withNumber(kHIDUsage_FIDO_U2FDevice, 32)},
      {"RegisterService", kOSBooleanTrue},
      {"HIDDefaultBehavior", kOSBooleanTrue},
      {"AppleVendorSupported", kOSBooleanTrue}};
...
kern_return_t USBDevice::getReport(IOMemoryDescriptor *report,
...
kern_return_t USBDevice::setReport(IOMemoryDescriptor *report,
                                   IOHIDReportType reportType,
                                   IOOptionBits options,
                                   uint32_t completionTimeout,
                                   OSAction *action) {
  Log("setReport(reportType: %d, completionTimeout: %u)", reportType,
      completionTimeout);
...
void USBDevice::sendReportFromDevice(IOMemoryDescriptor *report) {
  ...
  kern_return_t ret = handleReport(mach_absolute_time(), report,
